...
class AppraisalController extends Controller
{
	...
	public function index(Request $request)
	{
		...
		if ($all_emp[0]->count_no > 0) {
			...
			empty($request->period_no) ?: ($query .= " and g.period_no = ? " AND $qinput[] = $request->period_no);
		...
		//	empty($request->department_code) ?: ($query .= " and b.department_code = ? " AND $qinput[] = $request->department_code);
			...
			do {				
				...
				empty($request->period_no) ?: ($recur_sql .= " and g.period_no = ? " AND $rqinput[] = $request->period_no);
	...
	{
		$head = DB::select("
			SELECT b.emp_code, b.emp_name, b.working_start_date, b.position_name, b.section_name, b.department_name, b.chief_emp_code, e.emp_name chief_emp_name, c.appraisal_period_desc, d.appraisal_type_name, a.stage_id, f.status, a.result_score, f.edit_flag
			FROM emp_result a
			left outer join employee b
			on a.emp_code = b.emp_code
		...
		$items = DB::select("
			select b.appraisal_item_name, b.structure_id, c.structure_name, d.form_id, d.app_url, c.nof_target_score, a.*, e.perspective_name
	...
	public function update(Request $request, $emp_result_id)
	{
		...
		foreach ($request->appraisal as $a) {
			...
			if (empty($aresult)) {
				...
				$aresult->actual_value = $a['actual_value'];
