...
class AppraisalAssignmentController extends Controller
{
	...
	public function index(Request $request)
	{
		...
		if ($all_emp[0]->count_no > 0) {
			...
			$query_unassign .= "
				select distinct er.emp_result_id, er.status, er.emp_code, e.emp_name, er.department_name, er.section_name, t.appraisal_type_name, er.position_name, t.appraisal_type_id, p.period_id, concat(p.appraisal_period_desc,' Start Date: ',p.start_date,' End Date: ',p.end_date) appraisal_period_desc, e.is_coporate_kpi
				...
				and er.period_id = p.period_id
			";
			empty($request->position_code) ?: ($query_unassign .= " and er.position_code = ? " AND $qinput[] = $request->position_code);
			empty($request->department_code) ?: ($query_unassign .= " and er.department_code = ? " AND $qinput[] = $request->department_code);
			empty($request->emp_code) ?: ($query_unassign .= " and e.emp_code = ? " AND $qinput[] = $request->emp_code);	
						...
						foreach ($request->appraisal_items as $i) {
							if ($i['form_id'] == 1) {		
								if ($i['nof_target_score'] == 1) {
									...
									$aitem->department_code = $employee->department_code;
									$aitem->department_name = $employee->department_name;
									$aitem->section_code = $employee->section_code;
									$aitem->section_name = $employee->section_name;
									$aitem->position_code = $employee->position_code;
									$aitem->position_name = $employee->position_name;
	...
	{
		$head = DB::select("
			SELECT b.emp_code, b.emp_name, b.working_start_date, a.position_name, a.section_name, a.department_name, b.chief_emp_code, e.emp_name chief_emp_name, c.appraisal_period_desc, d.appraisal_type_name, a.stage_id, f.status, f.edit_flag, b.is_coporate_kpi
			FROM emp_result a
	...
	public function update(Request $request, $emp_result_id)
	{
		...
		$employee = Employee::find($emp_result->emp_code);
