...
class CommonDataSetController extends Controller
{
	...
	public function store(Request $request)
	{
		if ($request->is_sql == 1) {
			$validator = Validator::make($request->all(), [
				'cds_name' => 'required|max:100|unique:cds,cds_name,null,cds_id,appraisal_level_id,' . $request->appraisal_level_id,
				'cds_desc' => 'max:255',
	...
	public function update(Request $request, $cds_id)
	{
		...
		if ($request->is_sql == 1) {
			$validator = Validator::make($request->all(), [	
				'cds_name' => 'required|max:100|unique:cds,cds_name,'.$cds_id . ',cds_id,appraisal_level_id,' . $request->appraisal_level_id,
				'cds_desc' => 'max:255',
				'appraisal_level_id' => 'required|integer',
				'connection_id' => 'required|integer',
				'is_sql' => 'required|boolean',
