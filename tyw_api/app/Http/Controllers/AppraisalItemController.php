...
class AppraisalItemController extends Controller
{
	...
	{	
		$qinput = array();
		$query = "
			select s.seq_no, s.structure_name, s.structure_id, i.department_code, dep.department_name, i.appraisal_item_id, i.appraisal_item_name, l.appraisal_level_name,
			...
			on i.department_code = dep.department_code
			where 1=1
		";
		...
		empty($request->department_code) ?: ($query .= " And i.department_code = ? " AND $qinput[] = $request->department_code);
			...
			if (!isset($groups[$key])) {
				if ($s->form_name == 'Quantity') {
					$columns = [
						...
						[
							'column_display' => 'Department Name',
	...
	public function auto_appraisal_name(Request $request)
	{
		...
		empty($request->perspective_id) ?: ($query .= " and perspective_id = ? " AND $qinput[] = $request->perspective_id);
		...
		empty($request->appraisal_item_name) ?: ($query .= " and appraisal_item_name like ? " AND $qinput[] = '%'.$request->appraisal_item_name.'%');
	...
	public function store(Request $request)
	{
		...
		if ($request->form_id == 1) {
			$validator = Validator::make($request->all(), [	
				'appraisal_item_name' => 'required|max:255|unique:appraisal_item,appraisal_item_name,null,appraisal_item_id,department_code,' . $request->department_code . ',appraisal_level_id,' . $request->appraisal_level_id,
	...
	public function update(Request $request, $appraisal_item_id)
	{
		...
		if ($request->form_id == 1) {
			$validator = Validator::make($request->all(), [
				'appraisal_item_name' => 'required|max:255|unique:appraisal_item,appraisal_item_name,'.$appraisal_item_id . ',appraisal_item_id,department_code,' . $request->department_code . ',appraisal_level_id,' . $request->appraisal_level_id,
				'appraisal_level_id' => 'required|integer',
				'perspective_id' => 'required|integer',
				'structure_id' => 'required|integer',
				'department_code' => 'required|max:50',
