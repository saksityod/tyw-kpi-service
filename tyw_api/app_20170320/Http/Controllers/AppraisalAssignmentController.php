...
class AppraisalAssignmentController extends Controller
{
	...
	public function index(Request $request)
	{
		...
		if ($all_emp[0]->count_no > 0) {
			$query_unassign = "
				Select distinct null as emp_result_id,  'Unassigned' as status, emp_code, emp_name, department_name, section_name, 'N/A' as appraisal_type_name, position_name, null appraisal_type_id, 0 period_id, 'Unassigned' appraisal_period_desc
				From employee
				Where is_active = 1
			";
			...
			$query_unassign .= " GROUP  BY e.emp_code,
									  p.appraisal_year,
									  p.appraisal_frequency_id) assigned
					WHERE  assigned_total = period_total  ) union all ";
			
			$query_unassign .= "
				select distinct er.emp_result_id, er.status, er.emp_code, e.emp_name, e.department_name, e.section_name, t.appraisal_type_name, e.position_name, t.appraisal_type_id, p.period_id, concat(p.appraisal_period_desc,' Start Date: ',p.start_date,' End Date: ',p.end_date) appraisal_period_desc
